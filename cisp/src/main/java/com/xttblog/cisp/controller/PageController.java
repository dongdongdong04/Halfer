...
public class PageController {
  ...
  public String articleDeatil(HttpServletRequest request, @PathVariable(value = "id") Integer id){
    ...
    if(user==null||likeService.getByUserAndArticle(user.getId(),id)==null) {
      String tmp=articleVO.getContent();
      tmp=tmp.substring(0,tmp.length()/2)+"......(请点赞查看完整内容哦~)";
      articleVO.setContent(tmp);
    }
